...
import { updateOrder } from "../firebases/orders";
...
export default function UpdateOrderModal({ selectOrder }) {
    ...
    const formatDateTimeLocal = (timestamp) => {
            ...
            return `${yyyy}-${mm}-${dd}T${hh}:${min}`;  // format ที่ใช้กับ input type="datetime-local"
    };
    useEffect(() => {
        ...
        setCup(data.cupSize || '');
        setDate(formatDateTimeLocal(data.date) || '');
    ...
    const handleUpdate = async () => {
        ...
        const jsDate = new Date(date); // "2025-07-17T10:30"
        ...
        try {
            await updateOrder(selectOrder.id, newOrder);
    ...
    return (
        <div className="modal fade" id="updateOrder" tabIndex="-1" aria-hidden="true">
          <div className="modal-dialog modal-lg">
            <div className="modal-content border-0 shadow rounded-4 overflow-hidden">
              ...
              <div className="modal-body p-4" style={{ backgroundColor: '#f7f9fb' }}>
                ...
                {/* Date & Time */}
                    <div className="mb-3">
                    <label className="form-label">เลือกวันและเวลา</label>
                    <input
                        type="datetime-local"
              ...
              <div className="modal-footer bg-light border-0 p-4 d-flex justify-content-end gap-2">
                ...
                <button type="button" className="btn btn-success px-4" onClick={handleUpdate}>
                  <FaSave className="me-2" /> {isLoad?'กำลังอับเดต ... '  : 'อับเดต'}
                </button>
              </div>
